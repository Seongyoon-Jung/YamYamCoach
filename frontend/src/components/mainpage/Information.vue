                ...
                <div class="row gx-3 gy-4">
                    ...
                    <div class="col-6 text-center">
                        <div class="d-flex align-items-center justify-content-center mb-2">
                            <button class="btn btn-sm btn-outline-danger me-2 control-btn" @click="updateExerciseMinutes(-10)">
                                -
                            </button>
                            <div class="text-muted info-title mx-2">운동시간</div>
                            <button class="btn btn-sm btn-outline-success ms-2 control-btn" @click="updateExerciseMinutes(10)">
                                +
                            </button>
                        </div>
                    ...
                    <div class="col-6 text-center">
                        <div class="d-flex align-items-center justify-content-center mb-2">
                            <button class="btn btn-sm btn-outline-danger me-2 control-btn" @click="updateWaterIntake(-100)">
                                -
                            </button>
                            <div class="text-muted info-title mx-2">물 섭취</div>
                            <button class="btn btn-sm btn-outline-success ms-2 control-btn" @click="updateWaterIntake(100)">
                                +
                            </button>
                        </div>
                        <div class="d-inline-block position-relative" style="width:100px; height:100px;">
                    ...
                    <div class="col-12">
                        <div class="d-flex align-items-center justify-content-center mb-2">
                            <button class="btn btn-sm btn-outline-danger me-2 control-btn" @click="updateSleepMinutes(-30)">
                                -
                            </button>
                            <div class="text-muted info-title mx-2">오늘 수면 시간: {{ formatSleepTime(healthData.sleepMinutes) }}</div>
                            <button class="btn btn-sm btn-outline-success ms-2 control-btn" @click="updateSleepMinutes(30)">
                                +
                            </button>
                        </div>
...
const fetchTodayHealthLog = async () => {
    ...
    try {
        ...
        if (response.data && response.data.success && response.data.data) {
            ...
            error.value = response.data && response.data.message ? response.data.message : '데이터를 가져오는 데 실패했습니다.';
...
const updateExerciseMinutes = async (minutes) => {
    try {
        ...
        if (response.data && response.data.success && response.data.data) {
            ...
            console.error('운동 시간 업데이트 실패:', response.data ? response.data.message : '응답 형식 오류');
        }
    } catch (err) {
...
const updateWaterIntake = async (milliliters) => {
    try {
        ...
        if (response.data && response.data.success && response.data.data) {
            ...
            console.error('물 섭취량 업데이트 실패:', response.data ? response.data.message : '응답 형식 오류');
        }
    } catch (err) {
...
const updateSleepMinutes = async (minutes) => {
    try {
        ...
        if (response.data && response.data.success && response.data.data) {
            ...
            console.error('수면 시간 업데이트 실패:', response.data ? response.data.message : '응답 형식 오류');
        }
    } catch (err) {
