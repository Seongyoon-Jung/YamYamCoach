<template>
  ...
  >
    ...
    <!-- 레시피 생성 중 로딩 모달 -->
    <div v-if="isCreatingRecipe" class="recipe-loading-modal">
      <div class="recipe-loading-content">
        <div class="spinner-border text-primary mb-3" role="status">
          <span class="visually-hidden">로딩중...</span>
        </div>
        <h5 class="mb-2">레시피 생성 중...</h5>
        <p class="text-muted">{{ creatingRecipeName }} 레시피를 준비하고 있습니다.</p>
...
const searchInternalRecipe = async (foodName) => {
  try {
    ...
    // 검색 결과가 없는 경우 레시피 추가 시도
    console.log('내부 레시피 검색 결과 없음, 레시피 추가 시도')
    isCreatingRecipe.value = true
    creatingRecipeName.value = foodName
    
    const addRecipeResponse = await axios.post('http://localhost:5001/add-recipe', {
      foodName: foodName
    })
    
    if (addRecipeResponse.data.success) {
      console.log('레시피 추가 성공:', addRecipeResponse.data)
      return addRecipeResponse.data.recipeUrl
    }
...
.recipe-loading-content .spinner-border {
  width: 3rem;
  height: 3rem;
}

.recipe-loading-content h5 {
  color: #0d6efd;
  margin-bottom: 0.5rem;
}

.recipe-loading-content p {
  margin-bottom: 0.25rem;
}

.recipe-loading-content p.small {
  font-size: 0.875rem;
}
</style>
