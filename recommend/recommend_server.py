...
from searchRecipe import process_food_recipe, search_recipe, crawl_recipe, process_recipe_with_langchain, save_to_db
...
def add_recipe():
    ...
    try:
        ...
        logger.info(f"레시피 추가 요청 수신: {food_name}")
        ...
        if not recipe_url:
            return jsonify({
                "success": False,
                "message": f"'{food_name}'에 대한 레시피를 찾을 수 없습니다."
            }), 404
        ...
        if save_to_db(processed_data):
            ...
            try:
                with conn.cursor() as cursor:
                    cursor.execute("SELECT id FROM recipes WHERE name = %s", (processed_data['name'],))
        ...
        logger.exception("레시피 추가 중 오류 발생:")
        return jsonify({
            "success": False,
            "message": f"레시피 추가 중 오류가 발생했습니다: {str(e)}"
        }), 500
